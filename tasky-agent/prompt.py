...
6. Use the `get_datetime_now` tool to know the current date, time, and weekday if needed.
7. If user only provides weekday, use the `get_datetime_now` tool to get the current date, \
    time and weekday, and then calculate the next weekday date based on the current date.
